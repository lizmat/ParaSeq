...
my %lines = @filenames.&racify(1, :!auto).map: { $_ => .lines.List }
```
...
The arguments to `racify` are **exactly the same** as the arguments to `hyperize`. But calling `racify` will **NOT** ensure that the values produced will be in the same order as the values from which they were produced. Depending on the application, this can mean a small but potentially significant improvement of throughput.

Situations where one typically would use `racify`, is when the result is ultimately stored in a `Hash` or a `Map`, which does not have a predictable order of its elements anyway.
...
Bool. Returns `True` if racing semantics are used for delivering values. Otherwise `False` (indicating hyper semantics, which **does** guarantee the order of delivered values).
